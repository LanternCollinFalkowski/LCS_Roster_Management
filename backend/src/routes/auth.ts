...
import { OIDC_TX_COOKIE, beginSignIn, completeSignIn, isAllowedDomain, safeReturnTo } from "../auth/entra.js";
import { requireAuth } from "../auth/middleware.js";
import { appBaseUrl, devAuthEnabled, isProd, ssoConfigured } from "../env.js";
import { guestDomains } from "../services/settings.js";
import { audit } from "../services/audit.js";
import { ROLES, roleFor } from "../services/permissions.js";
 ...
 *
 * Nobody gets access just by signing in: an unknown account files an access
 * request, and an admin decides. An admin can also pre-invite an address (for
 * a partner's Google Workspace staff); that address is then admitted even if
 * its domain is not on the allow-list.
 */
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (!user && !isAllowedDomain(identity.email, await guestDomains())) {
        return failTo(res, `${identity.email} is not from an organisation that uses Lantern Roster.`);
      }
      ...
      if (!user) {
        const requester = await prisma.user.create({ data: { ...fromDirectory, roleKey: "viewer", status: "requested" } });
        await audit({
      ...
      if (user.status === "denied" || user.status === "deactivated") {
        return noticeTo(res, "You don't have access to Lantern Roster. If you think that's a mistake, contact the IT Team.");
      }
      ...
      return failTo(res, "Signed in with Microsoft, but Lantern Roster could not load your account. Please try again.");
    }
  })
);
...
authRouter.get("/roles", requireAuth, (_req, res) => {
  res.json(ROLES);
});
