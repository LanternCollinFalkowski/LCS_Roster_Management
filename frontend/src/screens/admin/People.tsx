...
type Draft = { id?: string; name: string; email: string; roleKey: string; status: UserStatus; siteIds: string[] };
...
export function AdminPeople() {
  ...
  const edit = (u: ManagedUser) =>
    setDraft({ id: u.id, name: u.name, email: u.email, roleKey: u.roleKey, status: u.status, siteIds: u.sites.map((s) => s.id) });
  ...
  return (
    <Page>
      <PageHeader
        title="People & roles"
        subtitle="Invite Lantern and partner staff. Partner staff can sign in with Google Workspace once their domain or address is admitted."
        actions={
          <Button onClick={() => setDraft({ name: "", email: "", roleKey: "staff", status: "invited", siteIds: [] })}>
            ...
            <DialogBody className="grid gap-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Field label="Role" hint={role?.description}>
                  <Select value={draft.roleKey} onChange={(e) => setDraft({ ...draft, roleKey: e.target.value })}
                    options={(roles ?? []).map((r) => ({ value: r.key, label: r.name }))} />
                </Field>
                {draft.id && (
                  <Field label="Status">
                    <Select value={draft.status} onChange={(e) => setDraft({ ...draft, status: e.target.value as UserStatus })}
                      options={[
              ...
              {role?.allSites ? (
                <p className="rounded-input bg-subtle px-3 py-2 text-[13px] text-muted">Administrators see every site.</p>
              ) : (
                <Field
                  label={
                  ...
                  hint="They'll only see — and only be able to change — the rosters of these sites."
                >
