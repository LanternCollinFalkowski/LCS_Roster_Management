...
/** The review threshold and friends. */
      ...
      <Card className="p-5">
        <Field label="Review after (hours without activity)" hint="Someone lands in the review queue when no form has named them and nobody has confirmed them for this long. Sites can override it (Admin → Sites) — shelters may want 24.">
          <div className="flex gap-2">
            <Input type="number" min={1} max={1440} value={hours} onChange={(e) => setHours(e.target.value)} className="w-32" />
            <Button onClick={save} disabled={!hours || hours === data?.attentionHours}>Save</Button>
          </div>
        </Field>
